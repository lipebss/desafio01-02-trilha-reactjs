...
export function CartProvider({ children }: CartProviderProps): JSX.Element {
  ...
  const removeProduct = (productId: number) => {
    try {
      ...
      const productExistsCart = productCart.findIndex(product => product.id === productId)

      if (productExistsCart >= 0) {
        productCart.splice(productExistsCart, 1)
  ...
  const updateProductAmount = async ({
    ...
    try {
      ...
      const productExists = updateProductCart.find(product => product.id === productId)
